...
class WaggleDanceDataset(Dataset):
    def __init__(self, dataset_name, img_size, len_clip, transform, split, centered_clip):
        ...
        # Load image list
        df = pd.read_csv(os.path.join(self.path_data, 'annotations.csv'))
        self.video_list = df.loc[df['split'] == self.split, 'filename'].unique().tolist()
        frame_list = []
...
def build_dataset(parameters, transform, split,):
