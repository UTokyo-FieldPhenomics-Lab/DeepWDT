...
def parse_args():
    ...
    #                     help='base lr.')
    parser.add_argument('-lr', '--base_lr', default= 1e-5, type=float, 
                        help='base lr.')
    parser.add_argument('-minlr', '--min_lr_ratio', default=0.25, type=float, 
                        help='min_lr = base_lr*min_lr_ratio.')
    ...
    parser.add_argument('--warmup_epoch', nargs='+', default=1, type=int,
                        help='epoch to increase lr from 0 to base_lr')
    parser.add_argument('--no_decrease_lr_epoch', nargs='+', default=7, type=int,
                        help='at this epoch, the lr will not change')
...
def train():
    ...
    dataset, _, num_classes = build_dataset(d_cfg, args, is_train=True)
    ...
    no_decrease_lr_epoch = args.no_decrease_lr_epoch
    accumulate = args.accumulate
    optimizer, start_epoch = build_optimizer(d_cfg, model_without_ddp, 0, args.resume)
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        path_to_save = os.path.join('runs/training/weights', f'{version}_K{args.len_clip}')
