...
class TRAINING_DATASET_Evaluator(object):
    def __init__(self,
                 data_root=None,
        ...
        if eval_split:
            self.gt_folder = os.path.join(gt_folder, eval_split)
        else:
        ...
        if 'test' in self.gt_folder:
            ...
            self.testlist = os.path.join(data_root, 'splitfiles/vallist01.txt')

        # dataset
        if metric == 'fmap':
            self.testset = Training_Dataset(
                data_root=data_root,
                dataset=dataset,
                img_size=img_size,
                transform=transform,
